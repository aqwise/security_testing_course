...
const quizQuestions = [
    ...
    { question: "Какая функция в MySQL позволяет создать временную задержку?", answers: ["WAIT()", "SLEEP()", "DELAY()", "PAUSE()"], correctAnswerIndex: 1 },
    { question: "Что такое Blind SQL injection?", answers: ["Инъекция без использования SQL", "SQL-инъекция, при которой результаты запроса не отображаются", "Инъекция только в скрытые поля", "Автоматическая SQL-инъекция"], correctAnswerIndex: 1 },
    { question: "Какой символ в Oracle SQL используется для конкатенации строк?", answers: ["+", "&", "||", "CONCAT"], correctAnswerIndex: 2 },
    { question: "Что означает полезная нагрузка ' UNION SELECT NULL,NULL,NULL--?", answers: ["Удаление трех столбцов", "Проверка количества столбцов в исходной таблице", "Создание трех новых записей", "Установка значений в NULL"], correctAnswerIndex: 1 },
    { question: "Какая функция PostgreSQL возвращает версию базы данных?", answers: ["VERSION()", "@@VERSION", "version()", "Все перечисленные"], correctAnswerIndex: 2 },
    { question: "Что такое Error-based SQL injection?", answers: ["Инъекция, вызывающая только ошибки", "Использование ошибок базы данных для извлечения информации", "Инъекция в поля с ошибками", "Атака на логи ошибок"], correctAnswerIndex: 1 },
    { question: "Какой оператор MongoDB эквивалентен SQL LIKE?", answers: ["$like", "$regex", "$match", "$search"], correctAnswerIndex: 1 },
    { question: "Что делает полезная нагрузка '; DROP TABLE users;--?", answers: ["Создает таблицу users", "Удаляет таблицу users", "Очищает данные в таблице users", "Переименовывает таблицу users"], correctAnswerIndex: 1 },
    { question: "Какая техника используется для определения количества столбцов в UNION-атаке?", answers: ["Добавление NULL значений", "Использование ORDER BY", "Подсчет запятых", "Все перечисленные"], correctAnswerIndex: 3 },
    { question: "Что такое Time-based SQL injection?", answers: ["Инъекция, зависящая от времени суток", "Использование временных задержек для подтверждения инъекции", "Атака только во время работы", "Инъекция в поля времени"], correctAnswerIndex: 1 },
    { question: "Какой HTTP-заголовок часто содержит SQL-инъекции?", answers: ["Content-Type", "User-Agent", "Accept", "Host"], correctAnswerIndex: 1 },
    { question: "Что означает полезная нагрузка ' AND SUBSTRING(@@version,1,1)='5'--?", answers: ["Проверяет первый символ версии", "Удаляет первый символ", "Заменяет версию на 5", "Устанавливает версию"], correctAnswerIndex: 0 },
    { question: "Какая функция MS SQL Server возвращает имя текущей базы данных?", answers: ["DATABASE()", "DB_NAME()", "CURRENT_DB()", "SCHEMA()"], correctAnswerIndex: 1 },
    { question: "Что такое Boolean-based SQL injection?", answers: ["Инъекция в булевы поля", "Использование логических условий для извлечения данных", "Инъекция значений true/false", "Атака на условные операторы"], correctAnswerIndex: 1 },
    { question: "Какой символ в MS SQL Server используется для комментариев?", answers: ["#", "//", "--", "/**/"], correctAnswerIndex: 2 },
    { question: "Что делает функция LOAD_FILE() в MySQL?", answers: ["Загружает данные в таблицу", "Читает содержимое файла с сервера", "Создает новый файл", "Удаляет файл"], correctAnswerIndex: 1 },
    { question: "Какой оператор NoSQL MongoDB эквивалентен SQL OR?", answers: ["$or", "$and", "$either", "$union"], correctAnswerIndex: 0 },
    { question: "Что такое Out-of-band SQL injection?", answers: ["Инъекция вне диапазона", "Использование внешних каналов для получения данных", "Инъекция в отключенной базе", "Атака через другие протоколы"], correctAnswerIndex: 1 },
    { question: "Какая функция Oracle позволяет выполнить HTTP-запрос?", answers: ["HTTP_GET()", "UTL_HTTP", "WEB_REQUEST()", "CURL()"], correctAnswerIndex: 1 },
    { question: "Что означает INFORMATION_SCHEMA в контексте SQL-инъекций?", answers: ["Схема для информации", "Системная база данных с метаданными", "Таблица пользователей", "Схема безопасности"], correctAnswerIndex: 1 },
    { question: "Какой тип данных лучше всего подходит для хранения паролей?", answers: ["VARCHAR", "TEXT", "HASH", "Хешированные строки"], correctAnswerIndex: 3 },
    { question: "Что такое WAF в контексте защиты от SQL-инъекций?", answers: ["Wide Area Function", "Web Application Firewall", "Write After Filter", "Web Access Filter"], correctAnswerIndex: 1 },
    { question: "Какая функция MySQL позволяет получить список баз данных?", answers: ["SHOW DATABASES", "LIST_SCHEMAS()", "GET_DBS()", "DATABASES()"], correctAnswerIndex: 0 },
    { question: "Что означает полезная нагрузка ' OR UPDATEXML(1,CONCAT(0x7e,@@version,0x7e),1)--?", answers: ["Обновляет XML", "Вызывает ошибку с выводом версии", "Создает XML документ", "Удаляет XML данные"], correctAnswerIndex: 1 },
    { question: "Какой метод лучше всего защищает от SQL-инъекций?", answers: ["Фильтрация входных данных", "Подготовленные запросы (Prepared Statements)", "Шифрование данных", "Ограничение прав пользователей"], correctAnswerIndex: 1 },
    { question: "Что такое sqlmap?", answers: ["Карта базы данных", "Автоматизированный инструмент для эксплуатации SQL-инъекций", "Схема SQL запросов", "Протокол безопасности"], correctAnswerIndex: 1 },
    { question: "Какая полезная нагрузка может помочь определить СУБД через ошибки?", answers: ["' AND 1=1--", "' AND 1=CONVERT(int,'test')--", "' OR 1=1--", "' UNION SELECT NULL--"], correctAnswerIndex: 1 },
    { question: "Что означает термин 'Stored Procedure' в контексте SQL-инъекций?", answers: ["Сохраненная процедура, которая может быть уязвима", "Процедура сохранения данных", "Процедура хранения паролей", "Алгоритм шифрования"], correctAnswerIndex: 0 },
    { question: "Какой HTTP-метод чаще всего используется для SQL-инъекций?", answers: ["GET", "POST", "PUT", "Все перечисленные"], correctAnswerIndex: 3 },
    { question: "Что такое Stacked Queries в SQL-инъекциях?", answers: ["Запросы в стеке", "Выполнение нескольких SQL-запросов подряд", "Запросы с ошибками", "Медленные запросы"], correctAnswerIndex: 1 },
    { question: "Какая функция SQLite возвращает версию базы данных?", answers: ["VERSION()", "sqlite_version()", "SQLITE_VERSION()", "DB_VERSION()"], correctAnswerIndex: 1 },
    { question: "Что означает техника 'SQL Injection via Cookies'?", answers: ["Инъекция через куки", "Кража куки через SQL", "Шифрование куки", "Создание куки"], correctAnswerIndex: 0 },
    { question: "Какой инструмент НЕ используется для тестирования SQL-инъекций?", answers: ["Burp Suite", "SQLMap", "Nmap", "OWASP ZAP"], correctAnswerIndex: 2 },
    { question: "Что такое Second-Order SQL injection?", answers: ["Повторная SQL-инъекция", "Инъекция, выполняющаяся в два этапа", "Инъекция во второй базе данных", "Вторичная атака"], correctAnswerIndex: 1 },
    { question: "Какая кодировка может помочь обойти фильтры WAF?", answers: ["Base64", "URL-кодирование", "Hex-кодирование", "Все перечисленные"], correctAnswerIndex: 3 }
];
...
export default function ModuleFourLessonOnePage() {
  ...
  return (
    <ContentPageLayout 
      title="Урок 1: Атака на хранилища данных"
    >
      <div className="space-y-8">
        ...
        <section>
          ...
          <P><strong>ORDER BY</strong> – если параметр управляет сортировкой, злоумышленник может подставить имя столбца или вложенный подзапрос. Подстановка числа 1, 2, 3 меняет порядок сортировки.</P>
          
          <P><strong>INSERT, UPDATE и DELETE</strong> – изменение структуры запросов позволяет добавлять, менять или удалять записи. Например, инъекция <code>'; DROP TABLE users;--'</code> может удалить таблицу.</P>
          
          <P><strong>Union‑атаки</strong> – оператор UNION позволяет объединять результаты нескольких запросов. Если число и тип столбцов совпадают, злоумышленник может внедрить второй запрос и вывести его результаты.</P>
          ...
          <P><strong>Отправка математических выражений:</strong> Вместо числа 2 подставляют <code>1+1</code> или <code>3-1</code>. Если результат не меняется, возможно, база вычисляет выражение.</P>
          
          <P><strong>Использование функций:</strong> База воспринимает функции <code>ASCII()</code> или <code>CHAR()</code> как числа. Замена 2 на <code>67-ASCII('A')</code> позволяет обойти фильтры одинарных кавычек.</P>
          
          <P><strong>URL‑кодирование:</strong> При отправке атакующих строк важно кодировать проблемные символы: <code>&</code> → <code>%26</code>, <code>=</code> → <code>%3d</code>, пробел → <code>%20</code>.</P>
          
          <P><strong>Логические выводы:</strong> Если приложение не показывает ошибки, можно использовать условия, изменяющие время отклика.</P>

          <H3>3.4 Снятие отпечатков базы данных</H3>
          <P>Разные СУБД по‑разному конкатенируют строки и выполняют функции:</P>
          
          <P><strong>Oracle:</strong> <code>'serv'||'ices'</code>, <code>BITAND(1,1)-BITAND(1,1)</code></P>
          
          <P><strong>MS‑SQL:</strong> <code>'serv'+'ices'</code>, <code>@@PACK_RECEIVED-@@PACK_RECEIVED</code></P>
          
          <P><strong>MySQL:</strong> <code>'serv' 'ices'</code>, <code>CONNECTION_ID()-CONNECTION_ID()</code></P>
        ...
        <section>
          ...
          <H3>4.1 NoSQL (MongoDB)</H3>
          <P>
            Типичный запрос имеет вид <code>db.users.find({`{ username: user, password: pass }`})</code>. 
            Если параметры передаются как JSON‑строка, можно передать объект <code>{`{ "$ne": null }`}</code>, 
            чтобы условие всегда было истинным.
          </P>

          <H3>4.2 XPath</H3>
          <P>
            При запросе <code>//user[username/text()='$user' and password/text()='$pass']</code> можно ввести 
            <code>admin' or '1'='1</code> и получить первый узел пользователя.
          </P>

          <H3>4.3 LDAP</H3>
          <P>
            Каталоги LDAP используют фильтры вида <code>(uid=$user)(userPassword=$pass)</code>. 
            Инъекция <code>*)(|(uid=*))</code> вернёт всех пользователей.
          </P>
        </section>
