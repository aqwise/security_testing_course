...
export default function Module2Lesson1Page() {
  return (
    ...
    >
      ...
      <Card className="my-6">
        ...
        <CardContent>
          ...
          <Ul items={[
            ...
            <>Проверить наличие и значения флагов HttpOnly, Secure, SameSite<Link href="#source-9" className={LinkStyle}><sup>9</sup></Link>.</>
          ]}/>
          ...
          <P><strong>Контекст OWASP Top 10:</strong> Неправильная конфигурация флагов cookie относится к A05:2021-Security Misconfiguration<Link href="#source-6" className={LinkStyle}><sup>6</sup></Link>. Если токены сессии слабые или предсказуемые, это также может затрагивать A02:2021-Cryptographic Failures.</P>
          <P>Отсутствие флага Secure позволяет передавать cookie по незащищенному HTTP-соединению, что делает их уязвимыми для перехвата. Отсутствие флага HttpOnly позволяет JavaScript-коду на странице получить доступ к cookie, что критично при XSS-атаках<Link href="#source-9" className={LinkStyle}><sup>9</sup></Link>. Флаг SameSite помогает защититься от CSRF-атак, ограничивая отправку cookie с межсайтовыми запросами<Link href="#source-10" className={LinkStyle}><sup>10</sup></Link>. Анализ этих флагов является важным шагом в оценке безопасности управления сессиями.</P>
        </CardContent>
      </Card>
      ...
      <P>
        Уязвимости IDOR возникают, когда приложение доверяет идентификаторам объектов, передаваемым пользователем, без должной проверки прав доступа текущего пользователя к запрашиваемому объекту<Link href="#source-12" className={LinkStyle}><sup>12</sup></Link>.
      </P>
      <Card className="my-6">
        <CardHeader>
        ...
        <CardContent>
          ...
          <P>Уязвимость IDOR существует из-за того, что приложение доверяет предоставленным пользователем идентификаторам без достаточных проверок авторизации на стороне сервера для этого конкретного ресурса и пользователя. Злоумышленники эксплуатируют это, просто изменяя идентификатор. Техника HPP, описанная в <Link href="#source-14" className={LinkStyle}><sup>14</sup></Link>, демонстрирует более продвинутый способ обхода простых проверок, когда сервер некорректно обрабатывает дублирующиеся параметры. IDOR-уязвимости чрезвычайно распространены и могут приводить к значительным утечкам данных или несанкционированным действиям, подчеркивая критическую необходимость применения проверок авторизации в каждой точке, где доступ к данным или функциям осуществляется на основе контролируемых пользователем идентификаторов.</P>
        </CardContent>
      </Card>
      ...
      <P>
        Черный список определяет запрещенные символы или паттерны. Этот подход часто оказывается неэффективным, так как атакующие постоянно находят способы обхода таких списков, используя различные кодировки или не учтенные разработчиком векторы<Link href="#source-17" className={LinkStyle}><sup>17</sup></Link>.
      </P>
      <P>
      ...
      <P>
        Эта таблица наглядно демонстрирует различия между белыми и черными списками, опираясь на определения из WAHH2 и примеры обхода из <Link href="#source-17" className={LinkStyle}><sup>17</sup></Link> и<Link href="#source-18" className={LinkStyle}><sup>18</sup></Link>. Она служит кратким руководством для понимания, почему белый список является предпочтительной стратегией безопасности, напрямую поддерживая основную идею урока по обработке ввода.
      </P>
      ...
      <Card className="my-6">
        ...
        <CardContent>
          ...
          <Ul items={[
            <>Отраженный XSS: Ввод немедленно отражается на странице (например, в результатах поиска, сообщении об ошибке<Link href="#source-15" className={LinkStyle}><sup>15</sup></Link>).</>,
            <>Хранимый XSS: Вредоносный скрипт сохраняется на сервере (например, в базе данных, в профиле пользователя, комментарии) и затем отображается другим пользователям<Link href="#source-13" className={LinkStyle}><sup>13</sup></Link>.</>,
            <>DOM XSS: Уязвимость в клиентском коде; полезная нагрузка может не достигать сервера. Манипуляция происходит в Document Object Model (DOM) браузера<Link href="#source-15" className={LinkStyle}><sup>15</sup></Link>.</>
          ]}/>
          <P><strong>Воздействие:</strong> Кража cookie, перехват сессий, дефейсмент, доставка вредоносного ПО<Link href="#source-15" className={LinkStyle}><sup>15</sup></Link>.</P>
      ...
      <Card className="my-6">
        ...
        <CardContent>
          ...
          <P><strong>Шаги (на основе <Link href="#source-32" className={LinkStyle}><sup>32</sup></Link>):</strong></P>
          <Ul items={[
        ...
        <Table>
          ...
          <TableBody>
            <TableRow>
              ...
              <TableCell>A07: Identification and Authentication Failures <Link href="#source-7" className={LinkStyle}><sup>7</sup></Link></TableCell>
            ...
            <TableRow>
              ...
              <TableCell>A05: Security Misconfiguration (для флагов); A02: Cryptographic Failures (если токены слабые/предсказуемые<Link href="#source-9" className={LinkStyle}><sup>9</sup></Link>)</TableCell>
            ...
            <TableRow>
              ...
              <TableCell>A01: Broken Access Control <Link href="#source-11" className={LinkStyle}><sup>11</sup></Link></TableCell>
            ...
            <TableRow>
              ...
              <TableCell>A03: Injection <Link href="#source-8" className={LinkStyle}><sup>8</sup></Link></TableCell>
            ...
            <TableRow>
              ...
              <TableCell>A01: Broken Access Control (если доступ к неавторизованным файлам); A03: Injection (если ведет к RCE<Link href="#source-22" className={LinkStyle}><sup>22</sup></Link>)</TableCell>
              <TableCell>1.3 Обработка Пользовательского Ввода</TableCell>
            </TableRow>
            <TableRow>
              <TableCell>Детализированные Сообщения об Ошибках</TableCell>
              <TableCell>Утечка SQL-ошибки в Juice Shop</TableCell>
              <TableCell>A05: Security Misconfiguration (Утечка информации – форма неверной конфигурации<Link href="#source-25" className={LinkStyle}><sup>25</sup></Link>)</TableCell>
            ...
            <TableRow>
              ...
              <TableCell>A05: Security Misconfiguration <Link href="#source-32" className={LinkStyle}><sup>32</sup></Link></TableCell>
