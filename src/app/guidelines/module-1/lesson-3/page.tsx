...
export default function Module1Lesson3Page() {
  return (
    ...
    >
      ...
      <Ul items={[
        "Современный стек технологий: В отличие от DVWA, Juice Shop имитирует архитектуру современных одностраничных приложений (SPA) с REST API, что делает его более реалистичной целью для тренировки.",
        <>Геймификация: Процесс обучения построен в игровой форме. За успешное решение заданий (эксплуатацию уязвимостей) начисляются очки, прогресс отслеживается на специальной "Доске результатов" (Score Board). Приложение уведомляет пользователя о решенных задачах.<sup>24</sup></>,
        <>Hacking Instructor: Для некоторых заданий доступен встроенный "Инструктор по взлому", который предоставляет пошаговые подсказки и руководства, что очень полезно для новичков.<sup>24</sup></>,
        <>Разнообразие заданий: Juice Shop содержит большое количество заданий (более 100) с различными уровнями сложности, от очень простых (1 звезда) до экспертных (6 звезд).<sup>25</sup></>,
        "Открытый исходный код: Как и многие проекты OWASP, Juice Shop является опенсорсным, что позволяет изучать его код для лучшего понимания уязвимостей.",
      ]} />
      <P>
        Название "Juice Shop" (Магазин соков) имеет интересное происхождение. Немецкое слово "Saftladen", означающее "захудалое заведение" или "бесполезная контора", дословно переводится как "магазин соков".<sup>24</sup>
      </P>
      <P>
      ...
      <Ul items={[
        <><code className="font-mono">docker run</code>: команда для запуска нового контейнера.</>,
        <><code className="font-mono">--rm</code>: флаг, который указывает Docker автоматически удалить контейнер после его остановки.</>,
        <><code className="font-mono">-d</code>: запускает контейнер в фоновом (detached) режиме, то есть терминал не будет "занят" логами контейнера.</>,
        <><code className="font-mono">-p 3000:3000</code>: пробрасывает порт 3000 внутри контейнера (на этом порту работает Juice Shop) на порт 3000 вашей хост-машины.</>,
        <><code className="font-mono">bkimminich/juice-shop</code>: имя образа, из которого запускается контейнер.</>,
      ]} />
      ...
      <Ul items={[
        ...
        <>В адресной строке введите: <code className="font-mono text-sm">http://localhost:3000</code></>
      ]} />
      <P>
      ...
      <P>
        Почему это важно? Без доступа к Score Board вы не сможете видеть, какие задания существуют, какие вы уже решили, и какие категории уязвимостей представлены в приложении.
      </P>
      <H3>Методы поиска Score Board (применяем навыки Разведки и Картирования из WAHH2):</H3>
      <P>
        Согласно официальному руководству "Pwning OWASP Juice Shop" и общему описанию проекта, Score Board "тщательно спрятан"<sup>28</sup> и на него нет прямой, очевидной ссылки в пользовательском интерфейсе приложения.<sup>28</sup> Это сделано намеренно, чтобы первое задание было связано с исследованием.
      </P>
      <P>Вот несколько подходов к поиску:</P>
      <H3>Угадывание URL (URL guessing):</H3>
      <Ul items={[
        <>Зная, что Score Board существует, можно попробовать угадать его URL-адрес. Распространенные пути для подобных страниц могут быть <code className="font-mono text-sm">/score-board</code>, <code className="font-mono text-sm">/scoreboard</code>, <code className="font-mono text-sm">/challenges</code>, <code className="font-mono text-sm">/results</code> и т.п..<sup>28</sup> Попробуйте ввести эти варианты в адресную строку после <code className="font-mono text-sm">http://localhost:3000</code>.</>
      ]} />
      <H3>Анализ клиентского кода (HTML, JavaScript):</H3>
      <Ul items={[
        "Современные веб-приложения, особенно SPA (Single Page Applications) как Juice Shop, активно используют JavaScript для навигации и отображения контента.",
        ...
        <>В одном из видео-разборов<sup>29</sup> упоминается, что ссылка на scoreboard может быть найдена в виде <code className="font-mono text-sm">routerLink</code> в коде Angular-компонентов.</>,
      ...
      <H3>Использование Hacking Instructor:</H3>
      <Ul items={[
        <>Если при первом запуске или в процессе навигации вы видите приветственный баннер или всплывающие подсказки с иконкой "🎓" (академическая шапочка), это Hacking Instructor. Он может предложить помощь в поиске Score Board.<sup>25</sup> Следуйте его инструкциям.</>
      ]} />
      <H3>Просмотр HTTP-трафика в Burp Suite:</H3>
      <Ul items={[
        "Хотя прямую ссылку может быть сложно найти, анализ HTTP-запросов и ответов в Burp Suite (вкладка Proxy -> HTTP history) при навигации по сайту может иногда выявить вызовы API или загрузку скриптов, которые содержат информацию о путях к различным компонентам приложения, включая Score Board."
      ]} />
      <H3>Конкретный путь (наиболее вероятный):</H3>
      <P>Чаще всего Score Board в OWASP Juice Shop доступен по URL-адресу:</P>
      <P><code className="font-mono text-sm bg-muted p-1 rounded">http://localhost:3000/#/score-board</code></P>
      <P>
        После того как вы успешно перейдете по правильному адресу, приложение поздравит вас с решением вашего первого задания – "Score Board". Сам процесс поиска Score Board является отличным обучающим упражнением, знакомящим пользователя с методами начальной разведки и анализа клиентской части веб-приложения. Это не просто "найти страницу", а применить навыки анализа HTML/JS и угадывания URL, что является частью этапов "Картирование" и "Обнаружение" из методологии WAHH2. Наличие "Hacking Instructor"<sup>25</sup> для этого первого ключевого шага помогает предотвратить "застревание" новичка на самом старте и обеспечивает положительный первый опыт.
      </P>
      ...
      <Ul items={[
        <><strong>"Score Board" (Категория: Miscellaneous)</strong><br />Описание: Найти тщательно спрятанную страницу 'Score Board'.<br />Это задание вы уже должны были решить, чтобы получить доступ к списку заданий.</>,
        <><strong>"Bonus Payload" (Категория: XSS)</strong><br />Описание: Использовать специальную XSS-нагрузку (payload), чтобы отобразить встроенный контент. Точная нагрузка обычно указана в описании задания на Score Board или в подсказках Hacking Instructor. Согласно <sup>31</sup>, это: <br /><code>{`<iframe width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/771984076&color=%23ff5500&auto_play=true&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true"></iframe>`}</code><br />Где пробовать: Поле поиска на главной странице Juice Shop – это классическое место для тестирования XSS-уязвимостей.<sup>30</sup><br />Действия:<br />1. Скопируйте указанную выше XSS-нагрузку.<br />2. Вставьте ее в поле поиска на главной странице Juice Shop.<br />3. Нажмите Enter или кнопку поиска.<br />Результат: Если уязвимость эксплуатируется, на странице должен появиться встроенный плеер SoundCloud. Juice Shop уведомит вас о решении задания.<br />Hacking Instructor: Для этого задания доступен Hacking Instructor (иконка "🎓" на Score Board рядом с названием задания).<sup>25</sup> Нажмите на нее для получения пошаговых инструкций.</>,
        <><strong>"DOM XSS" (Категория: XSS)</strong><br />Описание: Выполнить атаку DOM-based Cross-Site Scripting, используя указанную нагрузку. Согласно <sup>31</sup> и <sup>32</sup>, это: <br /><code>{`<iframe src="javascript:alert('xss')">`}</code><br />Где пробовать: Снова поле поиска.<br />Действия:<br />1. Скопируйте нагрузку <code>{`<iframe src="javascript:alert('xss')">`}</code>.<br />2. Вставьте ее в поле поиска на главной странице.<br />3. Нажмите Enter или кнопку поиска.<br />Результат: На странице должно появиться всплывающее окно JavaScript alert с текстом "xss". Задание будет отмечено как решенное.<br />Hacking Instructor: Для этого задания также доступен Hacking Instructor.<sup>25</sup></>,
        <><strong>"Error Handling" (Категория: Error Handling)</strong><br />Описание: Спровоцировать ошибку в приложении, которая обрабатывается не очень корректно или неконсистентно.<sup>31</sup><br />Где пробовать: Различные поля ввода, параметры URL.<br />Действия (пример):<br />1. Попробуйте ввести специальные символы, такие как одинарная кавычка ('), двойная кавычка ("), или последовательности символов, характерные для SQL-инъекций (например, <code className="font-mono text-sm">' OR 1=1--</code>) в поле поиска или в других полях ввода на сайте.<sup>30</sup><br />2. Манипулируйте параметрами в URL, добавляя некорректные значения.<br />Результат: Появление нестандартной страницы ошибки, утечка информации о сервере или базе данных, или любое другое необычное поведение приложения, которое свидетельствует о плохой обработке ошибок.<br />Hacking Instructor: Проверьте наличие иконки "🎓".</>
      ]} />
      <P>
        При выполнении этих заданий активно используйте Burp Suite (Proxy -> HTTP history) для наблюдения за тем, какие запросы отправляются на сервер и какие ответы приходят. Это поможет вам лучше понять, как ваши действия в браузере транслируются в HTTP-взаимодействия.
      </P>
      <P>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow>
              ...
              <TableCell>Угадывание URL (/#/score-board), анализ клиентского кода, подсказки Hacking Instructor.</TableCell>
            ...
            <TableRow>
              ...
              <TableCell>Поле поиска. Нагрузка: <code>{`<iframe...`}</code><sup>31</sup></TableCell>
              <TableCell>Да<sup>25</sup></TableCell>
            </TableRow>
            <TableRow>
      ...
      <Card className="my-6">
        <CardContent className="p-6 space-y-4">
          <div>
            ...
            <P><em>Правильный ответ: b) Node.js, Express и Angular (JavaScript-стек)</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: b) Страница, отслеживающая прогресс в решении заданий и перечисляющая их.</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: c) docker run --rm -d -p 3000:3000 bkimminich/juice-shop</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: c) {"<iframe src=\"javascript:alert('xss')\">"}</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: c) Для этого задания доступен Hacking Instructor (пошаговое руководство).</em></P>
          </div>
        </CardContent>
      </Card>
    </ContentPageLayout>
