...
export default function Module1Lesson3Page() {
  return (
    ...
    >
      ...
      <P>
        OWASP Juice Shop – это современное, намеренно уязвимое веб-приложение, разработанное проектом OWASP (Open Worldwide Application Security Project). Оно написано с использованием JavaScript-стека технологий: Node.js на сервере, фреймворк Express, и Angular на клиенте. Juice Shop идеально подходит для отработки навыков поиска и эксплуатации уязвимостей из списка OWASP Top 10, а также многих других, менее распространенных, но реальных проблем безопасности.
      </P>
      ...
      <Ul items={[
        ...
        <><strong>Геймификация:</strong> Процесс обучения построен в игровой форме. За успешное решение заданий (эксплуатацию уязвимостей) начисляются очки, прогресс отслеживается на специальной "Доске результатов" (Score Board). Приложение уведомляет пользователя о решенных задачах.</>,
        <><strong>Hacking Instructor:</strong> Для некоторых заданий доступен встроенный "Инструктор по взлому", который предоставляет пошаговые подсказки и руководства, что очень полезно для новичков.</>,
        <><strong>Разнообразие заданий:</strong> Juice Shop содержит большое количество заданий (более 100) с различными уровнями сложности, от очень простых (1 звезда) до экспертных (6 звезд).</>,
      ...
      <P>Название "Juice Shop" (Магазин соков) имеет интересное происхождение. Немецкое слово "Saftladen", означающее "захудалое заведение" или "бесполезная контора", дословно переводится как "магазин соков".</P>
      <Card className="my-4 p-4 border-l-4 border-primary bg-primary/10">
        <P>
          Juice Shop представляет собой более реалистичную и сложную цель по сравнению с DVWA. Его современный технологический стек (JavaScript-фреймворки, REST API) делает его отличным следующим шагом в обучении после освоения основ на DVWA. Элементы геймификации и наличие "Hacking Instructor" значительно повышают вовлеченность и доступность для новичков, помогая преодолевать трудности при столкновении со сложными задачами и снижая фрустрацию.
        </P>
      </Card>
      ...
      <P>
        Если вы еще не установили Docker, вернитесь к <Link href="/guidelines/module-1/lesson-1" className={LinkStyle}>Уроку 1.1</Link> и следуйте инструкциям по установке Docker Desktop для вашей операционной системы.
      </P>
      ...
      <P>Эта команда загрузит официальный образ, поддерживаемый разработчиками Juice Shop.</P>
      ...
      <P>
        Вы должны увидеть главную страницу интернет-магазина соков. При первом запуске может появиться приветственный баннер или всплывающее окно с предложением помощи от Hacking Instructor. Вы можете закрыть его или следовать его указаниям, если хотите.
      </P>
      <Card className="my-4 p-4 border-l-4 border-primary bg-primary/10">
        <P>
          Стандартизация развертывания через Docker делает Juice Shop доступным на любой платформе с минимальными усилиями. Это полностью соответствует его цели быть широко используемым учебным инструментом, так как Docker устраняет проблемы совместимости и сложности ручной установки Node.js и всех зависимостей Juice Shop, особенно для пользователей, не знакомых с экосистемой Node.js.
        </P>
      </Card>

      <H2>C. Навигация по Juice Shop: Поиск Score Board (применяем навыки Разведки и Картирования из WAHH2)</H2>
      <P>
        Одним из первых и самых важных заданий в Juice Shop является поиск Score Board (Доски результатов). Это не просто элемент интерфейса, а само по себе задание начального уровня сложности (обычно 1 звезда). Score Board отслеживает ваш прогресс в решении заданий (взломе уязвимостей) и показывает полный список доступных челленджей с их сложностью и категориями.
      </P>
      ...
      <H3>Методы поиска Score Board (применяем навыки Разведки и Картирования из WAHH2 (описанной в <Link href="/guidelines/module-1/lesson-1" className={LinkStyle}>Уроке 1.1</Link>)):</H3>
      <P>Согласно официальному руководству "Pwning OWASP Juice Shop" и общему описанию проекта, Score Board "тщательно спрятан" и на него нет прямой, очевидной ссылки в пользовательском интерфейсе приложения. Это сделано намеренно, чтобы первое задание было связано с исследованием.</P>
      <P>Вот несколько подходов к поиску:</P>
      <Ul items={[
        <><strong>Угадывание URL (URL guessing):</strong>
          <P className="mb-1">Зная, что Score Board существует, можно попробовать угадать его URL-адрес. Распространенные пути для подобных страниц могут быть <code>/score-board</code>, <code>/scoreboard</code>, <code>/challenges</code>, <code>/results</code> и т.п. Попробуйте ввести эти варианты в адресную строку после <code>http://localhost:3000</code>.</P>
        </>,
        <><strong>Анализ клиентского кода (HTML, JavaScript):</strong>
          ...
          <Ul items={[
              ...
              <>В одном из видео-разборов упоминается, что ссылка на scoreboard может быть найдена в виде <code>routerLink</code> в коде Angular-компонентов.</>,
              <>В старом, но все еще релевантном по принципу, разборе указывалось на поиск закомментированной ссылки в исходном коде главной страницы.</>
          ]} />
        </>,
        <><strong>Использование Hacking Instructor:</strong>
          <P className="mb-1">Если при первом запуске или в процессе навигации вы видите приветственный баннер или всплывающие подсказки с иконкой "🎓" (академическая шапочка), это Hacking Instructor. Он может предложить помощь в поиске Score Board. Следуйте его инструкциям.</P>
        </>,
      ...
      <H3>Рекомендуемые 1-звездочные задания для начала:</H3>
      <div className="space-y-6">
        ...
        <div>
          ...
          <P><strong>Описание:</strong> Использовать специальную XSS-нагрузку (payload), чтобы отобразить встроенный контент. Точная нагрузка обычно указана в описании задания на Score Board или в подсказках Hacking Instructor. Это:</P>
          ...
          <P><strong>Где пробовать:</strong> Поле поиска на главной странице Juice Shop – это классическое место для тестирования XSS-уязвимостей.</P>
          ...
          <P><strong>Hacking Instructor:</strong> Для этого задания доступен Hacking Instructor (иконка "🎓" на Score Board рядом с названием задания). Нажмите на нее для получения пошаговых инструкций.</P>
        </div>
        <div>
          <H3>"DOM XSS" (Категория: XSS)</H3>
          <P><strong>Описание:</strong> Выполнить атаку DOM-based Cross-Site Scripting, используя указанную нагрузку. Это: <code>{'<iframe src="javascript:alert(\'xss\')">'}</code></P>
          ...
          <P><strong>Hacking Instructor:</strong> Для этого задания также доступен Hacking Instructor.</P>
        </div>
        <div>
          <H3>"Error Handling" (Категория: Error Handling)</H3>
          <P><strong>Описание:</strong> Спровоцировать ошибку в приложении, которая обрабатывается не очень корректно или неконсистентно.</P>
          <P><strong>Где пробовать:</strong> Различные поля ввода, параметры URL.</P>
          <P><strong>Действия (пример):</strong></P>
          <Ul items={[
            <>Попробуйте ввести специальные символы, такие как одинарная кавычка ('), двойная кавычка ("), или последовательности символов, характерные для SQL-инъекций (например, <code>' OR 1=1--</code>) в поле поиска или в других полях ввода на сайте.</>,
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>Поле поиска. Нагрузка: <code>{'<iframe... src="https://w.soundcloud.com/...></iframe>'}</code>.</TableCell>
              <TableCell>Да</TableCell>
            </TableRow>
            <TableRow>
              ...
              <TableCell>Поле поиска. Нагрузка: <code>{'<iframe src="javascript:alert(\'xss\')"></iframe>'}</code>.</TableCell>
              <TableCell>Да</TableCell>
            </TableRow>
            <TableRow>
