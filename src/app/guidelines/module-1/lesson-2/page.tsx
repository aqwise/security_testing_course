...
export default function Module1Lesson2Page() {
  return (
    ...
    >
      ...
      <P>
        Damn Vulnerable Web Application (DVWA) – это веб-приложение, написанное на PHP и использующее базу данных MySQL (или MariaDB), которое намеренно содержит множество известных уязвимостей. Оно было создано с несколькими целями:
      </P>
      <Ul items={[
      ...
      <P>Эта команда основана на информации из.</P>
      ...
      <Ul items={[
        <>Склонируйте репозиторий DVWA: <CodeBlock language="bash" code="git clone https://github.com/digininja/DVWA.git" /></>,
        <>Перейдите в созданную директорию: <CodeBlock language="bash" code="cd DVWA" /></>,
        <>Запустите DVWA с помощью Docker Compose: <CodeBlock language="bash" code="docker compose up -d" /> Флаг <code>-d</code> запускает контейнеры в фоновом режиме. В этом случае DVWA будет доступен по адресу <Link href="http://localhost:4280" target="_blank" rel="noopener noreferrer" className={LinkStyle}>http://localhost:4280</Link>.</>
      ]} />
      ...
      <P>Используйте следующие учетные данные по умолчанию для входа:</P>
      <Ul items={[
      ...
      <P>
        Теперь, когда DVWA запущено и настроено, давайте применим первые два этапа методологии WAHH2 (описанной в <Link href="/guidelines/module-1/lesson-1" className={LinkStyle}>Уроке 1.1</Link>) – Разведку и Картирование – к нашему приложению.
      </P>
      ...
      <Card className="my-4 p-4 border-l-4 border-primary bg-primary/10">
        <P>
          Даже в таком простом приложении, как DVWA, этап картирования с помощью Burp Suite может выявить скрытые параметры или особенности поведения, которые не очевидны из пользовательского интерфейса. Burp Suite делает видимыми все HTTP-взаимодействия, включая те, что инициированы AJAX-запросами или отправлены через формы, предоставляя более полную картину точек входа для атак.
        </P>
      </Card>
