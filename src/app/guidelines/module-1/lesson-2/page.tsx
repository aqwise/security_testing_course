...
export default function Module1Lesson2Page() {
  return (
    ...
    >
      ...
      <Ul items={[
        "Для специалистов по безопасности: предоставить легальную среду для оттачивания навыков и тестирования инструментов penetration testing.",
        "Для веб-разработчиков: помочь лучше понять процессы защиты веб-приложений, демонстрируя, как работают атаки.",
        "Для студентов и преподавателей: служить учебным пособием для изучения безопасности веб-приложений в контролируемой среде классной комнаты или самостоятельного обучения.<sup>20</sup>"
      ]} />
      <P>
      ...
      <P>
        Мы будем использовать Docker для быстрого и безопасного развертывания DVWA. Существует несколько Docker-образов для DVWA. Для простоты этого урока мы воспользуемся популярным образом kaakaww/dvwa-docker, который легко запускается одной командой. В качестве альтернативы можно использовать официальный образ digininja/DVWA, но он требует Docker Compose.
      </P>
      <H3>Шаг 1: Загрузка образа (если еще не загружен)</H3>
      <P>Откройте терминал или командную строку и выполните следующую команду, чтобы загрузить образ kaakaww/dvwa-docker (если вы не сделали этого ранее):</P>
      ...
      <Ul items={[
        ...
        "<code>-p 8080:80</code>: пробрасывает порт 80 внутри контейнера (стандартный HTTP-порт, на котором работает веб-сервер DVWA) на порт 8080 вашей хост-машины. Это означает, что вы сможете получить доступ к DVWA через http://localhost:8080.",
        "<code>kaakaww/dvwa-docker:latest</code>: имя и тег образа, который мы запускаем."
      ]} />
      <P><strong>Альтернативный запуск официального образа digininja/DVWA (требует Docker Compose):</strong></P>
      <Ul items={[
        ...
        <>Запустите DVWA с помощью Docker Compose: <CodeBlock language="bash" code="docker compose up -d" /><sup>21</sup> Флаг <code>-d</code> запускает контейнеры в фоновом режиме. В этом случае DVWA будет доступен по адресу http://localhost:4280.<sup>21</sup></>
      ]} />
      ...
      <Ul items={[
        "<code>http://localhost:8080</code> (если вы использовали образ kaakaww/dvwa-docker)",
        "<code>http://localhost:4280</code> (если вы использовали официальный образ digininja/DVWA)"
      ]} />
      ...
      <Ul items={[
        "В меню слева на странице DVWA выберите пункт \"DVWA Security\".<sup>22</sup>",
        ...
        "Выберите \"Low\" из списка и нажмите кнопку \"Submit\".<sup>22</sup>"
      ]} />
      <P>
        Теперь DVWA готово к исследованию и первым попыткам эксплуатации уязвимостей. Существование нескольких Docker-образов для DVWA (например, kaakaww/dvwa-docker, vulnerables/web-dvwa <sup>23</sup>, официальный digininja/DVWA) подчеркивает популярность этого инструмента. Однако это также требует от пользователя внимательности при выборе образа, например, проверки его актуальности и поддержки. Для новичка использование простой команды docker run может быть предпочтительнее, чем работа с Docker Compose на самом начальном этапе.
      </P>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Источник(и)</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            <TableRow>
              <TableCell>URL-адрес (для kaakaww/dvwa-docker)</TableCell>
              <TableCell><code>http://localhost:8080</code></TableCell>
              <TableCell>12</TableCell>
            </TableRow>
            <TableRow>
              <TableCell>URL-адрес (для digininja/DVWA через Compose)</TableCell>
              <TableCell><code>http://localhost:4280</code></TableCell>
      ...
      <P>И другие, в зависимости от версии.</P>
      <H3>Навигация по приложению:</H3>
      <Ul items={[
        "Перейдите по каждой ссылке в меню.",
        "Обращайте внимание на то, как меняются URL-адреса в браузере. Какие параметры появляются в URL? Например, при переходе на страницу SQL Injection, URL может выглядеть как <code>http://localhost:8080/vulnerabilities/sqli/</code>."
      ]} />
      ...
      <P>Введите в поле IP-адрес <code>127.0.0.1</code> (это адрес самого сервера, localhost) и нажмите "Submit". Вы должны увидеть вывод команды ping, показывающий обмен пакетами с 127.0.0.1.</P>
      ...
      <P>Для Linux-серверов (а DVWA в Docker часто работает на Linux-основе) в качестве разделителя можно использовать точку с запятой (<code>;</code>) или двойной амперсанд (<code>&&</code>).</P>
      <P>Попробуйте ввести следующее в поле IP-адреса: <code>127.0.0.1; ls -la</code> Эта команда сначала выполнит ping 127.0.0.1, а затем, если уязвимость присутствует, выполнит команду ls -la, которая выведет список файлов и директорий в текущей рабочей папке веб-сервера.</P>
      <P>Другой пример: <code>127.0.0.1 && whoami</code> Команда whoami выведет имя пользователя, от имени которого работает веб-сервер.</P>
      <P>Нажмите "Submit".</P>

      <H3>Шаг 4: Анализ результата</H3>
      <P>
        Если уязвимость Command Injection присутствует и успешно эксплуатируется, то на странице, под выводом команды ping, вы увидите результат выполнения вашей внедренной команды (например, список файлов или имя пользователя www-data). Это произошло потому, что приложение напрямую вставило ваш ввод в системную команду без должной очистки или экранирования специальных символов. Пример эксплуатации из <sup>22</sup> (<code>127.0.0.1; cat /etc/passwd</code>) демонстрирует, как можно прочитать системные файлы.
      </P>
      ...
      <Ul items={[
        ...
        "В браузере на странице Command Injection в DVWA введите <code>127.0.0.1</code> и нажмите \"Submit\".",
        "Запрос будет перехвачен в Burp Suite. Вы увидите что-то вроде: <code>GET /vulnerabilities/exec/?ip=127.0.0.1&Submit=Submit HTTP/1.1</code> (или POST-запрос, в зависимости от реализации DVWA).",
        "Щелкните правой кнопкой мыши по перехваченному запросу и выберите \"Send to Repeater\".",
        "Перейдите на вкладку Repeater. Здесь вы можете изменять параметр ip (например, на <code>127.0.0.1; id</code>) и нажимать \"Send\" для отправки модифицированного запроса на сервер, наблюдая за ответом в панели справа. Это позволяет более гибко и контролируемо тестировать различные варианты инъекций."
      ]} />
      <P>
      ...
      <Card className="my-6">
        <CardContent className="p-6 space-y-4">
          <div>
            ...
            <P><em>Правильный ответ: b) Предоставлять среду для изучения и практики эксплуатации веб-уязвимостей.</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: c) admin/password</em></P>
          </div>
          <hr />
          <div>
            ...
            <P><em>Правильный ответ: d) Low</em></P>
          </div>
          <hr />
          <div>
            <P><strong>4. Если вы ввели <code>127.0.0.1; id</code> в поле для IP-адреса на странице Command Injection в DVWA (на Linux-сервере) и увидели вывод команды id, что это означает?</strong></P>
            <Ul items={[
            ...
            <P><em>Правильный ответ: b) Приложение уязвимо к внедрению команд, и ваша команда id была выполнена.</em></P>
          </div>
          <hr />
          <div>
            <P><strong>5. Какой этап методологии WAHH2 наиболее активно применяется при попытке выполнить команду <code>127.0.0.1; id</code> в DVWA?</strong></P>
            <Ul items={[
              "a) Разведка",
              "b) Картирование",
              "c) Обнаружение",
            ...
            <P><em>Правильный ответ: d) Эксплуатация</em></P>
          </div>
        </CardContent>
      </Card>
    </ContentPageLayout>
