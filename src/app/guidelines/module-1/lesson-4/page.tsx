...
export default function Module1Lesson4Page() {
  return (
    ...
    >
      ...
      <P><strong>Результат:</strong> Запрос превратился бы в <code className="font-mono text-sm bg-muted p-1 rounded">SELECT * FROM users WHERE username = 'admin'--' AND password = '...'</code>. Часть запроса после -- (комментарий SQL) игнорируется, позволяя войти как 'admin' без пароля<Link href="#source-17" className={LinkStyle}><sup className="align-super text-xs">17</sup></Link>.</P>
      <P><strong>Полезная нагрузка:</strong> <code className="font-mono text-sm bg-muted p-1 rounded">' OR '1'='1</code></P>
      <P><strong>Результат:</strong> Запрос мог бы стать <code className="font-mono text-sm bg-muted p-1 rounded">SELECT * FROM users WHERE username = '' OR '1'='1--' AND password = '...'</code>. Условие 1=1 всегда истинно, что может позволить обойти проверку<Link href="#source-15" className={LinkStyle}><sup className="align-super text-xs">15</sup></Link>.</P>
      ...
      <Ul items={[
        ...
        <>Определение количества столбцов: Чтобы использовать UNION SELECT, нам нужно знать, сколько столбцов возвращает исходный запрос. Используйте <code className="font-mono text-sm bg-muted p-1 rounded">ORDER BY N--</code> для этого.
          <Ul items={[
            ...
            "Введите: <code className=\"font-mono text-sm bg-muted p-1 rounded\">1' ORDER BY 3--</code> (должна появиться ошибка, так как третьего столбца нет). Это означает, что исходный запрос возвращает 2 столбца<Link href=\"#source-16\" className={LinkStyle}><sup className=\"align-super text-xs\">16</sup></Link>."
          ]} />
        </>,
      ...
      <P>На среднем уровне DVWA для SQL-инъекций используется выпадающий список для выбора User ID, и запросы отправляются методом POST<Link href="#source-20" className={LinkStyle}><sup className="align-super text-xs">20</sup></Link>. Также применяется функция <code className="font-mono text-sm bg-muted p-1 rounded">mysql_real_escape_string()</code> для экранирования спецсимволов.</P>
      <P><strong>Шаги:</strong></P>
      <Ul items={[
      ...
      <Ul items={[
        <><strong>Параметризованные запросы (Prepared Statements):</strong> Это наиболее надежный метод. Запрос сначала определяется с плейсхолдерами, а пользовательские данные передаются отдельно. СУБД обрабатывает данные именно как данные, а не как исполняемый код<Link href="#source-10" className={LinkStyle}><sup className="align-super text-xs">10</sup></Link>.
        ...
        "<strong>Валидация входных данных:</strong> Проверяйте все данные от пользователя на соответствие ожидаемому формату, типу, длине (используйте \"белые списки\")<Link href=\"#source-8\" className={LinkStyle}><sup className=\"align-super text-xs\">8</sup></Link>.",
        "<strong>Принцип наименьших привилегий:</strong> Учетная запись, используемая приложением для доступа к БД, должна иметь только минимально необходимые права<Link href=\"#source-10\" className={LinkStyle}><sup className=\"align-super text-xs\">10</sup></Link>.",
        "<strong>Экранирование спецсимволов:</strong> Менее надежный метод, используется, если параметризованные запросы недоступны. Применяйте специфичные для СУБД функции экранирования<Link href=\"#source-10\" className={LinkStyle}><sup className=\"align-super text-xs\">10</sup></Link>.",
        "<strong>Web Application Firewalls (WAF):</strong> Могут обнаруживать и блокировать известные SQLi-атаки, но не являются панацеей и могут быть обойдены<Link href=\"#source-10\" className={LinkStyle}><sup className=\"align-super text-xs\">10</sup></Link>."
      ]} />
